...
BEGIN
   ...
   IF NOT EXISTS (SELECT FROM pg_tables WHERE tablename = 'permissions') THEN
       ...
       RAISE NOTICE 'Table permissions created with default permissions';
   END IF;
   ...
   IF NOT EXISTS (SELECT FROM pg_tables WHERE tablename = 'role_permissions') THEN
       ...
       -- Призначаємо всі права адміністратору
       ...
       RAISE NOTICE 'Table role_permissions created with admin permissions';
   END IF;
